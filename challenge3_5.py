...
def modifier():
        x = input("enter l'élement que vous modifier: ")
        if x in mes_liste['inventaire']:
            index == mes_liste['inventaire'].index(x)

            ni = input("enter le nouveau élament de inventaire: ")
            mes_liste.inventaire [index(x)]== ni

            np = input("enter le nouveau élament de produit: ") 
            mes_liste["produit"].index(x) == np 

            nq = input("enter le nouveau élament quantite: ")
            mes_liste["quantite"].index(x) == nq

            print(f"le nouveau élement::{mes_liste['inventaire'][ni]},{mes_liste['produit'][np]}, quantite:{mes_liste['quantite'][nq]}")
        else:
            print("Ce produit n'existe pas")
